...
CDrawSlantToolbar::CDrawSlantToolbar()
{
	...
	*m_szTitle = 0;
	...
	SetColors( MAXDWORD, MAXDWORD, MAXDWORD, MAXDWORD, MAXDWORD );
...
BOOL CDrawSlantToolbar::Draw( HWND hWnd, HDC hDC, LPRECT pRect)
{
	...
	// Create tip window
	if ( bCreateTips ) if ( !m_tooltip.Create( hWnd, AfxGetInstanceHandle() ) ) bCreateTips = FALSE;
...
//		m_bck.Aluminize( 2 );
	...
	// Box it in
	CGrDC::Box3d( hDC, pRect, 1, m_rgbLtBorder, m_rgbDkBorder );
	...
	while ( ( psti = (LPSLANTTOOLBARINFO)GetNext( psti ) ) != NULL )
	{
		...
		if ( psti->icon == NULL )
		{
			...
			{
				CGrDC dc( hDC, FALSE );
				CGrPen lt( PS_SOLID, 1, m_rgbLtBorder );
				CGrPen dk( PS_SOLID, 1, m_rgbDkBorder );
	...
	if ( !bVert )
	{
		COLORREF rgbCaption = CGrDC::ScaleColor(	m_rgbBck, m_rgbSelection, 200 );
...
BOOL CDrawSlantToolbar::Draw(LPSLANTTOOLBARINFO psti, HDC hDC, LPRECT pRect, BOOL bVert)
{
	...
	// Let user update
	SendUpdate( psti->cmd, (LPARAM)psti );

	// Add or remove flashing item
	if ( ( psti->f1 & STIF1_FLASH ) != 0 )
	{	LPSFlashInfo pFi = m_lstFlash.push_back( psti );
		if ( pFi ) 
		{	pFi->bVert = bVert;
		 ...
		 ( psti->f1 & STIF1_HILIGHTALWAYS ) != 0 )
	{
		...
		CGrDC::FillSolidRect( cap, cap, CGrDC::ScaleColor(	m_rgbBck, m_rgbSelection, 100 ) );
		cap.Texturize( 20 );
	...
	if ( psti->icon != NULL )
	{
		...
		if ( ( psti->f1 & STIF1_MOUSEROTATE ) == 0 )
		{
			...
			{
				if ( ( psti->f1 & STIF1_FLASH ) == 0 || m_bFlashState )
					CGrDC::DrawIcon( hDC, NULL, psti->pressed, ix, iy, ( psti->f1 & STIF1_DISABLED ) != 0 );
				else if ( psti->flash != NULL )
					CGrDC::DrawIcon( hDC, NULL, psti->flash, ix, iy, ( psti->f1 & STIF1_DISABLED ) != 0 );
...
BOOL CDrawSlantToolbar::DrawFlash(HDC hDc, LPRECT pRect)
{
/*
	// Redraw each flash item
	THList< LPSLANTTOOLBARINFO, SFlashInfo >::iterator it = NULL;
	while ( ( it = m_lstFlash.next( it ) ) != NULL )
	{		
		Draw( *it->key(), hDc, &(*it)->rect, (*it)->bVert );
...
BOOL CDrawSlantToolbar::NeedFlash()
{
	// Invert flash state
	...
	try
	{
		THList< LPSLANTTOOLBARINFO, SFlashInfo >::iterator it = NULL;
		while ( ( it = m_lstFlash.next( it ) ) != NULL )
			if ( it->key() && ( ( *it->key() )->f1 & STIF1_CHECKED ) != 0 )
				return TRUE;
...
void CDrawSlantToolbar::SetColors(COLORREF rgbBck, COLORREF rgbLtBorder, COLORREF rgbDkBorder, COLORREF rgbText, COLORREF rgbSelection)
{	
	if ( rgbBck != MAXDWORD ) m_rgbBck = rgbBck;
	else m_rgbBck = GetSysColor( COLOR_3DFACE );

	if ( rgbLtBorder != MAXDWORD ) m_rgbLtBorder = rgbLtBorder;
	else m_rgbLtBorder = GetSysColor( COLOR_3DHILIGHT );

	if ( rgbDkBorder != MAXDWORD ) m_rgbDkBorder = rgbDkBorder;
	else m_rgbDkBorder = GetSysColor( COLOR_3DSHADOW );

	if ( rgbSelection != MAXDWORD ) m_rgbSelection = rgbSelection;
	else m_rgbSelection = GetSysColor( COLOR_ACTIVECAPTION );

	if ( rgbText != MAXDWORD ) m_rgbText = rgbText;
	else m_rgbText = GetSysColor( COLOR_BTNTEXT );
}
