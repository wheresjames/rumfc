...
CDlgEditParam::CDlgEditParam()
{
	...
	*m_szPath = 0;
	...
	ZeroMemory( m_rhIcon, sizeof( m_rhIcon ) );
}
...
BEGIN_MESSAGE_MAP(CDlgEditParam, CWnd)
	...
	switch( m_type )
	{
		...
		case PROPEDIT_FILE :
		{
			...
			if ( pButton != NULL )
			{
				...
				pButton->SetIcon( m_rhIcon[ eDirClosed ] );
		...
		case PROPEDIT_RGB :
		{
			...
			if ( pButton != NULL )
			{
				...
				pButton->SetIcon( m_rhIcon[ eColor ] );
		...
		case PROPEDIT_FONT :
		{
			...
			if ( pButton != NULL )
			{
				...
				pButton->SetIcon( m_rhIcon[ eFont ] );
...
//	if ( !Create( IDD, pWnd ) ) return FALSE;
	// Create the window
	CRect rect( 0, 0, 0, 0 );
	if ( !CWnd::Create( NULL, "ParamEdit", WS_CHILD | WS_VISIBLE, rect, pWnd, 0 ) )
		return FALSE;
...
BOOL CDlgEditParam::FillCombo()
{
	...
	::SendMessage( m_hWndCallback, m_uMsgCombo, (WPARAM)m_prop, (LPARAM)m_ctrl[ 0 ] );
